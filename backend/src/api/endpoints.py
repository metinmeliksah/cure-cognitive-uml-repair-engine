...
try:
    ...
    UMLMultiAgentSystem = None
    AI_AGENT_IMPORT_ERROR = exc
else:
    AI_AGENT_IMPORT_ERROR = None
...
def otonom_onarim(girdi: OtonomOnarimGirdisi):
    ...
    if not compile_sonuc["basarili"]:
        try:
            if UMLMultiAgentSystem is None:
                raise RuntimeError(f"AI agent import edilemedi: {AI_AGENT_IMPORT_ERROR}")
            ...
            # Ajan istekten gelen guvenli iterasyon limitiyle calisir.
            aktif_kod = agent.run(
                original_text=srs_metni,
                initial_uml=aktif_kod,
                max_iterations=girdi.max_iterasyon,
            )
            ...
            fallback_compile = _compile_test(aktif_kod)
            iterasyonlar.append({
                "iteration_no": min(girdi.max_iterasyon, len(iterasyonlar) + 1),
                "status": "FALLBACK_HEALER_APPLIED",
                "compile_result": fallback_compile,
                "fix_summary": "AI ajan calisamadigi icin deterministik basit healer uygulandi."
            })
