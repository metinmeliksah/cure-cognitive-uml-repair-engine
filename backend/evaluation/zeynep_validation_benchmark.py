"""
Zeynep backend validation benchmark.
...
def run_ocl_cases() -> list[dict]:
    ...
    for case in OCL_CASES:
        result = ocl_dogrula(case["plantuml"])
        passed = (
...
def run_semantic_cases() -> list[dict]:
    ...
    for case in SEMANTIC_CASES:
        result = semantik_sadakat_skoru(case["srs"], case["plantuml"])
        rows.append(
            {
                "id": case["id"],
                "description": case["description"],
...
def write_markdown_report(report: dict, output_path: Path) -> None:
    lines = [
        ...
        f"- rate={report['figure4_inferred_wilson']['rate']}",
        f"- lower={report['figure4_inferred_wilson']['lower_percent']}%",
        f"- upper={report['figure4_inferred_wilson']['upper_percent']}%",
        "",
...
def main() -> None:
    ...
    report = {
        "metadata": {
            ...
            "repair_experiment_log": "backend/evaluation/results/zeynep_repair_experiment_log.json",
            "repair_experiment_scope": "fresh deterministic backend repair run, not the original historical LLM-agent run",
        },
        "summary": {
        ...
        "figure4_inferred_wilson": wilson_interval(15, 20),
        "ocl_cases": ocl_rows,
    ...
    json_path = output_dir / "zeynep_validation_results.json"
    md_path = output_dir / "zeynep_validation_report.md"
